  ...
  try {
    ...
    return (
      <div className="mx-auto max-w-4xl p-6">
        ...
        <p className="text-red-600">Failed to load talents: {String(e?.message || e)}</p>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-4xl p-6">
      ...
      {talents.length === 0 ? (
        ...
        <ul className="space-y-3">
          {talents.map((t) => (
            <li key={t.id} className="border rounded-md p-4 hover:bg-black/5 dark:hover:bg-white/5 transition">
              <Link href={`/talents/${t.id}`} className="block">
                <div className="font-medium">{t.display_name || "Unnamed"}</div>
                <div className="text-sm text-gray-600 dark:text-gray-400">{t.headline || "—"}</div>
